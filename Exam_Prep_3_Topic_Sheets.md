...
6. Histogram → skutočná hustota pravdepodobnosti so rastúcim počtom vzoriek (zákon veľkých čísel).
...
parameter, ktorý odhadujeme, je štatistika vypočítaná z zašumených dát.
...
4. **Zákon $\sqrt{N}$**: chyba $\propto \sigma/\sqrt{N}$ → 4-násobok dát na polovičnú
   chybu. Preto priemerjeme opakované merania.
...
jednopárametrový prípad lineárnej regresie.
...
Gauss–Markovove predpoklady (šum s nulovým stredným hodnotou, konštantným rozptylom, nekorelovaný) $\Rightarrow$ **BLUE**.
...
3. **Kĺzavý priemer** je FIR dolnopriepustný filter; väčšie okno = viac vyhladzovanie.
...
5. Kompromis: viac vyhladzovanie odstraňuje šum, ale zároveň sploští skutočnú dynamiku.
